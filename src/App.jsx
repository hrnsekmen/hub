...
export default function App() {
  ...
  const reserve = (ev) => {
    ...
    if (g && ev.price > 0) {
      setGifts((arr) => arr.map((x) => (x.id === g.id ? { ...x, used: true } : x)))
    }
    setConfirmEv(ev)
  }
  ...
  const screen = () => {
    switch (tab) {
      case 'today': return <Today onOpen={openEvent} bookings={bookings} onTab={goTab} />
      case 'explore': return <Explore onOpen={openEvent} />
      case 'hum': return <MyHum bookings={bookings} gifts={gifts} onOpen={openEvent} onTab={goTab} />
      case 'profile': return <Profile bookings={bookings} />
      default: return null
    }
  }
  ...
  return (
    <div className="stage">
      <div className="phone">
        <div className="screen">
          ...
          <nav className="tabbar">
            {TABS.slice(0, 2).map((t) => (
              <button key={t.id} className={`tab ${tab === t.id && !detailId ? 'on' : ''}`} onClick={() => goTab(t.id)}>
                <span className="ic"><t.icon /></span>{t.label}
              </button>
            ))}

            <div className="tab tab-breathe">
              <button className="breathe-btn" aria-label="Breathe Hub" onClick={() => setBreathe(true)}>
                <span className="core" />
              </button>
        ...
        {/* Breathe Hub modal */}
        {breathe && <BreatheHub onClose={() => setBreathe(false)} onComplete={breatheComplete} />}
