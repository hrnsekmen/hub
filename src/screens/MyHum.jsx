...
export default function MyHum({ bookings, gifts, onOpen, onTab }) {
  ...
  const pct = Math.round((MEMBERSHIP.momentsThisSeason / MEMBERSHIP.seasonGoal) * 100)
  ...
  return (
    <div className="fade-screen">
      <div className="screen-head">
        ...
        <div className="display" style={{ fontSize: 30, marginTop: 6 }}>HUM’um</div>
      </div>

      <div className="subtabs" style={{ marginTop: 14 }}>
        <button className={`subtab ${tab === 'upcoming' ? 'on' : ''}`} onClick={() => setTab('upcoming')}>Yaklaşan</button>
        <button className={`subtab ${tab === 'moments' ? 'on' : ''}`} onClick={() => setTab('moments')}>Anlar</button>
        <button className={`subtab ${tab === 'history' ? 'on' : ''}`} onClick={() => setTab('history')}>Geçmiş</button>
      </div>
      ...
      {tab === 'upcoming' && (
        <>
          {upcoming.length === 0 ? (
            ...
            upcoming.map((e) => (
              <div className="brow" key={e.id} onClick={() => onOpen(e.id)} style={{ cursor: 'pointer' }}>
                <div className="bdate">
                ...
                <div className="bbody">
                  ...
                  <div className="bm">{e.time} · {e.place}</div>
                </div>
            ...
            <div className="pad">
              <div className="presence">
                {MEMBERSHIP.presence.map((v, i) => (
                  <span key={i} style={{ height: `${20 + (v / maxPresence) * 80}%`, opacity: v === 0 ? 0.3 : 0.85 }} />
                ))}
              </div>
            </div>
          </div>
            ...
            {gifts.map((g) => (
              <div className={`gift ${g.used ? 'used' : ''}`} key={g.id}>
                ...
                <div className="gift-body">
                  <div className="gl">{g.used ? 'Hediye kullanıldı' : g.label}</div>
                  <div className="gd">{g.used ? g.from : `${g.detail} · ${g.from}`}</div>
                </div>
              </div>
            ))}
          </div>
        ...
        <>
          ...
          {PAST_ATTENDED.map((p) => (
            <div className="erow" key={p.id} style={{ cursor: 'default' }}>
              ...
              <div className="erow-time"><Icon.check width={20} height={20} style={{ stroke: 'var(--sage-deep)' }} /></div>
            </div>
          ))}
        </>
      )}
